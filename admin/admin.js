...
const ROLE_PERMISSIONS = {
    'ChatMod':      ['players', 'mute', 'warn', 'check', 'logs', 'ai_chat'],
    'Pomocnik':     ['players', 'mute', 'warn', 'check', 'kick', 'logs', 'notes', 'ai_chat', 'stats_view'],
    'Moderator':    ['players', 'ban', 'mute', 'kick', 'warn', 'check', 'logs', 'notes',
                     'evidence_view', 'ip_view', 'ai_chat', 'stats_view', 'rank_manage'],
    'Admin':        ['players', 'ban', 'unban', 'mute', 'unmute', 'kick', 'warn', 'check',
                     'logs', 'notes', 'site', 'shop', 'media_manage', 'evidence_view', 'evidence_delete',
                     'ip_view', 'stats_view', 'rank_manage', 'lp_manage',
                     'ai_chat', 'ai_actions', 'plugins_view', 'stats_edit', 'cshop_manage'],
    'Zarządzający': ['all']
};
const ROLE_ORDER = ['ChatMod', 'Pomocnik', 'Moderator', 'Admin', 'Zarządzający'];
const PERMISSIONS_PL = {
    // ── Gracze i kary ────────────────────────────────────────────────────────
    players:            { label: '👥 Podgląd graczy',       desc: 'Może przeglądać listę graczy i ich profile.' },
    ...
    warn:               { label: '⚠️ Ostrzeżenia',           desc: 'Może nadawać ostrzeżenia i wysyłać tytuły.' },
    check:              { label: '🔍 Sprawdzanie',           desc: 'Może wykonywać kontrole (/check).' },
    notes:              { label: '📝 Notatki',               desc: 'Może dodawać notatki o graczach.' },
    ip_view:            { label: '🌐 Podgląd IP',            desc: 'Może widzieć adresy IP graczy w tabeli i profilu.' },
    rank_manage:        { label: '👑 Nadawanie rang',        desc: 'Może nadawać i zabierać rangi graczom (LuckPerms).' },
    lp_manage:          { label: '🔑 LuckPerms gracze',     desc: 'Może nadawać/odbierać uprawnienia LuckPerms graczom.' },
    // ── Logi i statystyki ────────────────────────────────────────────────────
    logs:               { label: '📋 Logi akcji',            desc: 'Może przeglądać logi administracyjne.' },
    stats_view:         { label: '📊 Statystyki adminów',   desc: 'Może przeglądać statystyki aktywności adminów.' },
    // ── Strona i media ───────────────────────────────────────────────────────
    site:               { label: '🌍 Zarządzanie stroną',   desc: 'Może edytować stronę (newsy, ankieta, propozycje, konkurs).' },
    shop:               { label: '🛒 Sklep webowy',          desc: 'Może nadawać produkty ze sklepu i przeglądać historię.' },
    media_manage:       { label: '🖼️ Media i pliki',         desc: 'Może zarządzać mediami i plikami dowodowymi.' },
    evidence_view:      { label: '📎 Podgląd dowodów',       desc: 'Może otwierać załączone pliki/linki.' },
    evidence_delete:    { label: '🗑️ Usuwanie dowodów',      desc: 'Może usuwać pliki dowodowe.' },
    // ── Pluginy ──────────────────────────────────────────────────────────────
    plugins_view:       { label: '🧩 Podgląd pluginów',     desc: 'Może widzieć zakładkę Pluginy (CStats, CShop, Połączenia).' },
    stats_edit:         { label: '✏️ Edycja CStats',         desc: 'Może edytować statystyki CStats graczy.' },
    cshop_manage:       { label: '🏪 CShop — itemy/ceny',   desc: 'Może edytować ceny i przedmioty w sklepie in-game.' },
    tax_manage:         { label: '💸 CShop — podatki',       desc: 'Może edytować progi podatkowe w CShop.' },
    // ── AI Asystent ──────────────────────────────────────────────────────────
    ai_chat:            { label: '💬 AI — czat',             desc: 'Może rozmawiać z AI Asystentem (zwykły czat, pytania).' },
    ai_actions:         { label: '🤖 AI — akcje admina',    desc: 'Może wykonywać akcje admina przez AI (ban, mute, rangi, itemy).' },
    ai_advanced:        { label: '⚡ AI — zaawansowane',    desc: 'Może używać zaawansowanych akcji AI (gamemode, tp, fly, god, speed).' },
    console:            { label: '💻 Konsola serwera',       desc: 'Może wysyłać dowolne komendy konsolowe przez AI (console_cmd).' },
    op_manage:          { label: '⭐ Zarządzanie OP',        desc: 'Może dawać/zabierać OP graczom.' },
    // ── Konta i uprawnienia panelu ───────────────────────────────────────────
    permissions_manage: { label: '🔐 Uprawnienia rang',     desc: 'Może edytować uprawnienia rang w panelu.' },
    admins_manage:      { label: '👤 Zarządzanie adminami', desc: 'Może tworzyć, edytować i usuwać konta adminów.' },
    // ── Pełny dostęp ─────────────────────────────────────────────────────────
    all:                { label: '⚡ Pełny dostęp',          desc: 'Ma pełny dostęp do wszystkich funkcji panelu.' }
};

// ─── SESSION TOKEN (zabezpieczenie przed console bypass) ─────────────────────
// Wszystkie funkcje sesji są zamknięte w IIFE — niedostępne z konsoli przeglądarki
const _sessionGuard = (() => {
    const SESSION_KEY      = 'critmc_panel_' + btoa('session_v2');
    const SESSION_DATA_KEY = 'critmc_panel_' + btoa('data_v2');
    let _token = null;
    ...
    function verify() {
        ...
        if (!stored || stored !== _token) return false;
        const raw = sessionStorage.getItem(SESSION_DATA_KEY);
        if (!raw) return false;
        try {
            const d = JSON.parse(atob(raw));
            return d._t === _token && !!d.l && (Date.now() - d.ts) < 8 * 3600_000; // max 8h sesja
        } catch { return false; }
    }
    ...
    function create(user) {
        ...
        sessionStorage.setItem(SESSION_DATA_KEY, btoa(JSON.stringify({ l: user.login, r: user.role, _t: _token, ts: Date.now() })));
    }
    ...
    return { verify, create, destroy };
})();

// MutationObserver — blokuje wstrzyknięcie klasy auth-ready bez ważnej sesji
const _authGuard = new MutationObserver(() => {
    if (document.body.classList.contains('auth-ready') && !_sessionGuard.verify()) {
        console.warn('[Security] Wykryto nieautoryzowane dodanie klasy auth-ready. Sesja unieważniona.');
        _sessionGuard.destroy();
        currentUser = null;
        document.body.classList.remove('auth-ready');
    }
});
_authGuard.observe(document.body, { attributes: true, attributeFilter: ['class'] });

// Wewnętrzne aliasy — NIE eksponowane globalnie
function _verifySession() { return _sessionGuard.verify(); }
function _createSession(user) { _sessionGuard.create(user); }
...
function hasPermission(perm) {
    // BEZPIECZEŃSTWO: sprawdź token sesji — samo currentUser !== null nie wystarczy
    if (!currentUser) return false;
    if (!_verifySession()) {
        console.warn('[Security] Sesja nieprawidłowa — wymuszam wylogowanie.');
        _destroySession();
        return false;
    }
...
function initPanelUI() {
    // Dodatkowe zabezpieczenie — sprawdź że sesja istnieje przed pokazaniem panelu
    if (!currentUser || !_verifySession()) {
        console.warn('[Security] initPanelUI wywołane bez sesji — blokuję.');
        return;
    }
...
// ─── Auto-wylogowanie po nieaktywności ────────────────────────────────────────
let _lastActivity = Date.now();
let _idleCheckInterval = null;

function _startIdleTimeout() {
    _lastActivity = Date.now();
    // Śledź aktywność (debounce 10s)
    let _actDebounce;
    const _resetActivity = () => {
        clearTimeout(_actDebounce);
        _actDebounce = setTimeout(() => { _lastActivity = Date.now(); }, 10000);
    };
    ...
    _idleCheckInterval = setInterval(() => {
        if (!_verifySession()) return;
        const timeoutMs = (parseInt(localStorage.getItem('critmc_idle_timeout') || '120')) * 60_000;
        if (Date.now() - _lastActivity > timeoutMs) {
            clearInterval(_idleCheckInterval);
            showToast('info', '⏰ Sesja wygasła z powodu nieaktywności — zaloguj się ponownie.');
...
function applyPermissions() {
    const p = (perm) => hasPermission(perm) || hasPermission('all');
    const show = (sel, condition) => {
        const el = document.querySelector(sel);
        if (el) el.style.display = condition ? '' : 'none';
    };

    // Nawigacja — widoczność przycisków w sidebarze
    show('.nav-btn[data-page="shop"]',       p('shop'));
    show('.nav-btn[data-page="files"]',      p('evidence_view'));
    show('.nav-btn[data-page="site"]',       p('site'));
    show('.nav-btn[data-page="permissions"]',p('permissions_manage'));
    show('.nav-btn[data-page="admins"]',     p('admins_manage'));
    show('.nav-btn[data-page="plugins"]',    p('plugins_view'));
    show('.nav-btn[data-page="ai"]',         p('ai_chat') || p('ai_actions'));
    show('.nav-btn[data-page="stats"]',      p('stats_view'));

    // Kolumna IP w tabeli graczy (ukryta dla ChatMod/Pomocnik bez ip_view)
    const ipHeader = document.querySelector('#players-tbody')?.closest('table')?.querySelectorAll('th')[1];
    // IP jest w renderPlayers — sprawdzimy uprawnienie przy renderowaniu

    // Przyciski zaawansowanych akcji AI — widoczność chipów
    const advChip = document.getElementById('ai-chip-advanced');
    if (advChip) advChip.style.display = p('ai_advanced') ? '' : 'none';
    const consoleChip = document.getElementById('ai-chip-console');
    if (consoleChip) consoleChip.style.display = p('console') ? '' : 'none';
}
...
window.loadSitePage = async function() {
    ...
    await Promise.all([siteLoadContestInfo(), siteLoadEntries(), siteLoadChanges(), siteLoadMedia()]);
};
...
async function loadCUstawieniaTab() {
    ...
    try {
        ...
        if (summary) summary.innerHTML = `
            <div class="cu-card" style="grid-column:1/-1;text-align:center;padding:1.5rem;">
                <div class="cu-title" style="color:#ef4444;justify-content:center;font-size:.9rem;margin-bottom:.6rem;">
                    <i class="fa-solid fa-triangle-exclamation"></i> Brak danych analitycznych
                </div>
                <div style="font-size:.85rem;color:var(--text-secondary);line-height:1.6;">
                    Kolekcja <code>server_analytics</code> nie istnieje w Firestore.<br>
                    Wymagany jest plugin <b>CUstawienia</b> zainstalowany na serwerze,<br>
                    który skanuje ekwipunki graczy i zapisuje wyniki do Firestore.<br><br>
                    <span style="color:#ef4444;font-size:.78rem;">Błąd: ${escapeHtml(e.message)}</span>
                </div>
                <button class="tbl-btn" onclick="loadCUstawieniaTab()" style="margin-top:.8rem;">
...
function _buildCShopTabHtml() {
    ...
    <div class="table-card" style="overflow:hidden;">
        <div style="padding:1rem 1.25rem;border-bottom:1px solid var(--border);">
            <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:.75rem;margin-bottom:.6rem;">
                <div style="font-size:.78rem;font-weight:800;color:var(--text-secondary);text-transform:uppercase;">
                    ...
                    <span id="cshop-hist-count" style="margin-left:.5rem;font-size:.72rem;color:var(--text-secondary);background:var(--bg);padding:.1rem .5rem;border-radius:6px;font-weight:700;">0 / 0</span>
                </div>
            ...
            <div style="display:flex;gap:.5rem;flex-wrap:wrap;align-items:center;">
                <input type="text" id="cshop-hist-search" placeholder="🔍 Gracz..." oninput="cshopFilterHistory()"
                    style="padding:.4rem .7rem;border:1.5px solid var(--border);border-radius:8px;font-size:.8rem;background:var(--bg);color:var(--text-primary);outline:none;font-family:var(--font);width:130px;">
                <input type="text" id="cshop-hist-item" placeholder="📦 Przedmiot..." oninput="cshopFilterHistory()"
                    style="padding:.4rem .7rem;border:1.5px solid var(--border);border-radius:8px;font-size:.8rem;background:var(--bg);color:var(--text-primary);outline:none;font-family:var(--font);width:130px;">
                ...
                <select id="cshop-hist-sort" onchange="cshopFilterHistory()"
                    ...
                    <option value="amount_desc">↓ Ilość</option>
                </select>
            </div>
        </div>
...
let _cshopHistoryLoaded = false;

function cshopNorm(value) {
    return String(value ?? '')
        .toLowerCase()
        .normalize('NFD')
        .replace(/[\u0300-\u036f]/g, '')
...
function cshopTimestampMs(value) {
    if (!value) return 0;
...
window.loadCShopItemsManager = async function() {
    ...
    try {
        ...
        let tabsHtml = `<button class="tbl-btn ${(!_cshopCurrentCategory || _cshopCurrentCategory === 'all') ? 'active' : ''}" data-cshop-category="all" onclick="_cshopSetCategory('all')" style="margin-right:.25rem;margin-bottom:.25rem;font-size:.7rem;">Wszystkie</button>`;
        categories.forEach(cat => {
            const isActive = _cshopCurrentCategory === cat;
            tabsHtml += `<button class="tbl-btn ${isActive ? 'active' : ''}" data-cshop-category="${escapeHtml(cat)}" onclick="_cshopSetCategory('${escapeHtml(cat)}')" style="margin-right:.25rem;margin-bottom:.25rem;font-size:.7rem;">${capitalize(cat)}</button>`;
        });
...
window._cshopSetCategory = function(cat) {
    ...
    if (tabsContainer) {
        ...
        buttons.forEach(btn => {
            const btnCat = btn.getAttribute('data-cshop-category') || '';
            const isAll = cat === 'all' && btnCat === 'all';
            const isCat = btnCat === cat;
...
function _renderCShopItemsTable() {
    ...
    const searchVal = cshopNorm(document.getElementById('cshop-items-search')?.value || '');
    if (searchVal) {
...
// Cache globalny transakcji — pobierany raz, używany przez wszystkie funkcje CShop
let _cshopAllTxns = null;
let _cshopTxnsLoadedAt = 0;
const _CSHOP_TXN_CACHE_MS = 30_000; // cache 30s

/** Pobierz transakcje (z cache) — pojedyncze źródło prawdy dla wszystkich funkcji CShop. */
async function _cshopGetTxns(force = false) {
    if (!force && _cshopAllTxns && (Date.now() - _cshopTxnsLoadedAt < _CSHOP_TXN_CACHE_MS)) {
        return _cshopAllTxns;
    }
    const snap = await getDocs(query(collection(db, 'cshop_transactions'), limit(500)));
    _cshopAllTxns = snap.docs.map(d => ({ id: d.id, ...d.data() })).sort((a, b) => {
        const ta = a.timestamp ? new Date(a.timestamp).getTime() : 0;
        const tb = b.timestamp ? new Date(b.timestamp).getTime() : 0;
        return tb - ta;
    });
    _cshopTxnsLoadedAt = Date.now();
    return _cshopAllTxns;
}
...
async function loadCShopDailyStats() {
    ...
    try {
        ...
        // Karty podsumowania — KLIKALNE, otwierają modal z pełnymi logami tego okresu
        const periods = [
            { label:'Ostatnia godzina', txns: txHour,  color:'#8b5cf6', ms: msHour },
            { label:'Dziś',             txns: txDay,   color:'#3b82f6', ms: msDay },
            { label:'Ten tydzień',      txns: txWeek,  color:'#10b981', ms: msWeek },
            { label:'Ten miesiąc',      txns: txMonth, color:'#f59e0b', ms: msMonth },
        ];

        // Zapisz zakresy globalnie do użycia przez modal
        window._cshopPeriods = periods;

        dailyEl.innerHTML = periods.map((p, idx) => {
            ...
            return `
            <div class="cshop-stat-card" style="border-top: 4px solid ${p.color};cursor:pointer;transition:transform .15s,box-shadow .15s;"
                 onmouseenter="this.style.transform='translateY(-3px)';this.style.boxShadow='0 8px 24px rgba(0,0,0,.2)';"
                 onmouseleave="this.style.transform='';this.style.boxShadow='';"
                 onclick="window.openCShopPeriodModal(${idx})">
                <div class="cshop-stat-title" style="color: ${p.color};">
                    <i class="fa-solid fa-calendar-day"></i> ${p.label}
                    <i class="fa-solid fa-arrow-up-right-from-square" style="font-size:.65rem;opacity:.5;margin-left:.3rem;"></i>
                </div>
        ...
        // Top kupowanych DZIŚ — klikalne, otwierają modal per-item z graczami
        const buyMap = {};
        txDay.filter(t => t.type === 'BUY').forEach(t => {
            const k = t.itemName || t.category || '?';
            if (!buyMap[k]) buyMap[k] = { count: 0, total: 0, players: new Set() };
            buyMap[k].count += (t.amount || 1);
            buyMap[k].total += (t.price  || 0);
            if (t.playerName) buyMap[k].players.add(t.playerName);
        });
        ...
        if (buyItems) {
            buyItems.innerHTML = topBuy.length ? topBuy.map(([item, d], i) =>
                `<div class="cshop-list-item" style="cursor:pointer;" onclick="window.openCShopItemModal('${escapeHtml(item)}','BUY')">
                    ...
                    <span style="color:#3b82f6;font-size:.72rem;background:rgba(59, 130, 246, 0.08);padding:0.15rem 0.4rem;border-radius:6px;font-weight:700;">${d.players.size} graczy</span>
                    <span style="color:var(--text-secondary);font-size:.75rem;background:rgba(59, 130, 246, 0.05);padding:0.15rem 0.4rem;border-radius:6px;font-weight:700;margin-left:.2rem;">${d.count} szt.</span>
        ...
        // Top sprzedawanych DZIŚ — klikalne
        const sellMap = {};
        txDay.filter(t => t.type === 'SELL').forEach(t => {
            const k = t.itemName || t.category || '?';
            if (!sellMap[k]) sellMap[k] = { count: 0, total: 0, players: new Set() };
            sellMap[k].count += (t.amount || 1);
            sellMap[k].total += (t.price  || 0);
            if (t.playerName) sellMap[k].players.add(t.playerName);
        });
        ...
        if (sellItems) {
            sellItems.innerHTML = topSell.length ? topSell.map(([item, d], i) =>
                `<div class="cshop-list-item" style="cursor:pointer;" onclick="window.openCShopItemModal('${escapeHtml(item)}','SELL')">
                    <span style="width:20px;text-align:center;font-size:.72rem;color:var(--text-secondary);font-weight:800;flex-shrink:0;">#${i+1}</span>
                    <span style="flex:1;font-weight:700;">${escapeHtml(item)}</span>
                    <span style="color:#10b981;font-size:.72rem;background:rgba(16, 185, 129, 0.08);padding:0.15rem 0.4rem;border-radius:6px;font-weight:700;">${d.players.size} graczy</span>
                    <span style="color:var(--text-secondary);font-size:.75rem;background:rgba(16, 185, 129, 0.05);padding:0.15rem 0.4rem;border-radius:6px;font-weight:700;margin-left:.2rem;">${d.count} szt.</span>
        ...
        // Topka zarabiających DZIŚ — klikalna (otwiera filtr po graczu)
        const earnMap = {};
        txDay.filter(t => t.type === 'SELL').forEach(t => {
            const k = t.playerName || '?';
            if (!earnMap[k]) earnMap[k] = { total: 0, uuid: t.uuid, count: 0 };
        ...
        if (topEarn) {
            ...
            topEarn.innerHTML = topEarners.length ? topEarners.map(([name, d], i) =>
                `<div class="cshop-list-item" style="cursor:pointer;" onclick="window.openCShopPlayerModal('${escapeHtml(name)}')">
                    ...
                    <span style="color:var(--text-secondary);font-size:.7rem;">${d.count} trans.</span>
                    <span style="font-weight:800;color:#10b981;margin-left:.3rem;">${d.total.toFixed(2)}$</span>
                </div>`).join('')
            : '<div style="text-align:center;padding:1.5rem;color:var(--text-secondary);font-size:.8rem;">Brak sprzedaży dziś</div>';
        }

        // Topka wydających DZIŚ — klikalna
        const spendMap = {};
        txDay.filter(t => t.type === 'BUY').forEach(t => {
            const k = t.playerName || '?';
            if (!spendMap[k]) spendMap[k] = { total: 0, count: 0 };
        ...
        if (topSpend) {
            ...
            topSpend.innerHTML = topSpenders.length ? topSpenders.map(([name, d], i) =>
                `<div class="cshop-list-item" style="cursor:pointer;" onclick="window.openCShopPlayerModal('${escapeHtml(name)}')">
                    ...
                    <span style="flex:1;font-weight:700;">${escapeHtml(name)}</span>
                    <span style="color:var(--text-secondary);font-size:.7rem;">${d.count} trans.</span>
                    <span style="font-weight:800;color:#ef4444;margin-left:.3rem;">${d.total.toFixed(2)}$</span>
                </div>`).join('')
            : '<div style="text-align:center;padding:1.5rem;color:var(--text-secondary);font-size:.8rem;">Brak kupna dziś</div>';
        }
...
// ─── MODALE CShop (nowe — otwierane z kart/list) ──────────────────────────────

/**
 * Modal okresu — pokazuje wszystkie transakcje z danego okresu (godzina/dzień/tydzień/miesiąc)
 * z pełną filtracją: nick, typ, przedmiot, sortowanie po $/ilości/dacie.
 */
window.openCShopPeriodModal = async function(periodIdx) {
    const periods = window._cshopPeriods || [];
    const p = periods[periodIdx];
    if (!p) return;
    const txns = p.txns;
    _cshopShowTxnModal({
        title: p.label + ' (' + txns.length + ' transakcji)',
        titleColor: p.color,
        txns: txns,
...
/** Modal konkretnego gracza — wszystkie jego transakcje. */
window.openCShopPlayerModal = async function(playerName) {
    const all = await _cshopGetTxns();
    const txns = all.filter(t => (t.playerName || '?').toLowerCase() === playerName.toLowerCase());
    _cshopShowTxnModal({
        title: '📋 ' + playerName + ' (' + txns.length + ' transakcji)',
        titleColor: '#8b5cf6',
        txns: txns,
...
/** Modal konkretnego przedmiotu — kto kupował/sprzedawał, ile, za ile. */
window.openCShopItemModal = async function(itemName, type) {
    const all = await _cshopGetTxns();
    const txns = all.filter(t => (t.itemName || t.category || '?') === itemName && (!type || t.type === type));
    const typeLabel = type === 'BUY' ? 'kupione' : type === 'SELL' ? 'sprzedane' : 'transakcje';
    _cshopShowTxnModal({
        title: '📦 ' + itemName + ' — ' + typeLabel + ' (' + txns.length + ')',
        titleColor: type === 'BUY' ? '#ef4444' : '#10b981',
        txns: txns,
        showPeriodFilter: true,
        lockedItem: itemName,
        showPlayerRanking: true  // dodatkowa sekcja: kto najwięcej tego kupił/sprzedał
    });
};

/**
 * Uniwersalny modal transakcji z filtrami i sortowaniem.
 * Opcje: { title, titleColor, txns, showPeriodFilter, lockedPlayer, lockedItem, showPlayerRanking }
 */
function _cshopShowTxnModal(opts) {
    // Stwórz modal jeśli nie istnieje
    let modal = document.getElementById('cshop-txn-modal');
    if (!modal) {
        modal = document.createElement('div');
        modal.id = 'cshop-txn-modal';
        modal.className = 'modal-overlay';
        modal.setAttribute('onclick', 'if(event.target===this)this.classList.remove(\'open\')');
        modal.innerHTML = `
            <div class="modal-box" style="max-width:95vw;width:1000px;max-height:92vh;">
                <div class="modal-header">
                    <div class="modal-title" id="cshop-txn-modal-title"></div>
                    <button class="modal-close" onclick="document.getElementById('cshop-txn-modal').classList.remove('open')"><i class="fa-solid fa-xmark"></i></button>
                </div>
                <div class="modal-body" id="cshop-txn-modal-body" style="max-height:80vh;overflow-y:auto;padding:1.2rem;"></div>
            </div>`;
        document.body.appendChild(modal);
    }
    const titleEl = document.getElementById('cshop-txn-modal-title');
    const bodyEl  = document.getElementById('cshop-txn-modal-body');
    if (titleEl) { titleEl.innerHTML = opts.title; titleEl.style.color = opts.titleColor || ''; }

    // Renderuj modal z filtrami
    _cshopRenderTxnModalBody(bodyEl, opts);
    modal.classList.add('open');
}

let _cshopModalState = null;

function _cshopRenderTxnModalBody(bodyEl, opts) {
    _cshopModalState = { txns: opts.txns, opts, search: '', type: '', sortBy: 'date_desc', periodMs: 0 };

    bodyEl.innerHTML = `
        <!-- Filtry -->
        <div style="display:flex;gap:.5rem;flex-wrap:wrap;margin-bottom:1rem;">
            ${!opts.lockedPlayer ? `
            <input type="text" id="cshop-modal-search" placeholder="🔍 Nick gracza..." autocomplete="off"
                style="flex:1;min-width:150px;padding:.55rem .8rem;border:1.5px solid var(--border);border-radius:8px;font-size:.85rem;background:var(--bg);color:var(--text-primary);font-family:var(--font);"
                oninput="_cshopModalState.search=this.value;_cshopFilterTxnModal()">` : ''}
            ${!opts.lockedItem ? `
            <input type="text" id="cshop-modal-item" placeholder="📦 Przedmiot..." autocomplete="off"
                style="flex:1;min-width:150px;padding:.55rem .8rem;border:1.5px solid var(--border);border-radius:8px;font-size:.85rem;background:var(--bg);color:var(--text-primary);font-family:var(--font);"
                oninput="_cshopModalState.item=this.value;_cshopFilterTxnModal()">` : ''}
            <select id="cshop-modal-type"
                style="padding:.55rem .6rem;border:1.5px solid var(--border);border-radius:8px;font-size:.82rem;background:var(--bg);color:var(--text-primary);font-family:var(--font);"
                onchange="_cshopModalState.type=this.value;_cshopFilterTxnModal()">
                <option value="">Wszystkie</option>
                <option value="BUY">Kupno</option>
                <option value="SELL">Sprzedaż</option>
            </select>
            ${opts.showPeriodFilter ? `
            <select id="cshop-modal-period"
                style="padding:.55rem .6rem;border:1.5px solid var(--border);border-radius:8px;font-size:.82rem;background:var(--bg);color:var(--text-primary);font-family:var(--font);"
                onchange="_cshopModalState.periodMs=parseInt(this.value);_cshopFilterTxnModal()">
                <option value="0">Cały czas</option>
                <option value="3600000">Ostatnia godzina</option>
                <option value="86400000">Ostatnie 24h</option>
                <option value="604800000">Ostatni tydzień</option>
                <option value="2592000000">Ostatni miesiąc</option>
            </select>` : ''}
            <select id="cshop-modal-sort"
                style="padding:.55rem .6rem;border:1.5px solid var(--border);border-radius:8px;font-size:.82rem;background:var(--bg);color:var(--text-primary);font-family:var(--font);"
                onchange="_cshopModalState.sortBy=this.value;_cshopFilterTxnModal()">
                <option value="date_desc">↓ Data (najnowsze)</option>
                <option value="date_asc">↑ Data (najstarsze)</option>
                <option value="price_desc">↓ Cena (najwyższa)</option>
                <option value="price_asc">↑ Cena (najniższa)</option>
        ...
        <!-- Sekcja: ranking graczy (jeśli showPlayerRanking) -->
        <div id="cshop-modal-ranking"></div>

        <!-- Podsumowanie -->
        <div id="cshop-modal-summary" style="display:flex;gap:.5rem;flex-wrap:wrap;margin-bottom:1rem;"></div>

        <!-- Tabela transakcji -->
        <div id="cshop-modal-list" style="max-height:50vh;overflow-y:auto;border:1px solid var(--border);border-radius:10px;"></div>
    `;

    _cshopFilterTxnModal();
}

window._cshopFilterTxnModal = function() {
    const state = _cshopModalState;
    if (!state) return;
    let txns = [...state.txns];
    ...
    if (state.search) {
        ...
        txns = txns.filter(t => (t.playerName || '').toLowerCase().includes(q));
    }
    // Filtr po przedmiocie
    if (state.item) {
        const q = state.item.toLowerCase();
        txns = txns.filter(t => (t.itemName || t.category || '').toLowerCase().includes(q));
    }
    // Filtr po typie
    if (state.type) txns = txns.filter(t => t.type === state.type);
    // Filtr po okresie
    if (state.periodMs > 0) {
        const cutoff = Date.now() - state.periodMs;
        txns = txns.filter(t => {
    ...
    txns.sort((a, b) => {
        const ta = a.timestamp ? new Date(a.timestamp).getTime() : 0;
        const tb = b.timestamp ? new Date(b.timestamp).getTime() : 0;
        if (sb === 'date_desc') return tb - ta;
        if (sb === 'date_asc')  return ta - tb;
        if (sb === 'price_desc') return (b.price||0) - (a.price||0);
        if (sb === 'price_asc')  return (a.price||0) - (b.price||0);
        if (sb === 'amount_desc') return (b.amount||0) - (a.amount||0);
        return 0;
    });

    // Podsumowanie
    const buyV  = txns.filter(t => t.type === 'BUY').reduce((s,t) => s + (t.price||0), 0);
    const sellV = txns.filter(t => t.type === 'SELL').reduce((s,t) => s + (t.price||0), 0);
    const totalAmt = txns.reduce((s,t) => s + (t.amount||1), 0);
    const uniqPlayers = new Set(txns.map(t => t.playerName)).size;
    const summaryEl = document.getElementById('cshop-modal-summary');
    if (summaryEl) {
        summaryEl.innerHTML = [
            ['fa-arrow-up-from-bracket', '#ef4444', 'Kupno', buyV.toFixed(2) + '$'],
            ['fa-arrow-down-to-bracket', '#10b981', 'Sprzedaż', sellV.toFixed(2) + '$'],
            ['fa-receipt', '#3b82f6', 'Transakcji', txns.length],
            ['fa-cubes-stacked', '#f59e0b', 'Sztuk', totalAmt],
            ['fa-users', '#8b5cf6', 'Graczy', uniqPlayers]
        ].map(([icon, color, label, val]) =>
            `<div style="background:var(--bg);border:1px solid var(--border);border-radius:8px;padding:.5rem .8rem;display:flex;align-items:center;gap:.5rem;flex:1;min-width:120px;">
                <i class="fa-solid ${icon}" style="color:${color};"></i>
                <div><div style="font-size:.62rem;color:var(--text-secondary);text-transform:uppercase;font-weight:700;">${label}</div>
                <div style="font-weight:800;color:${color};">${val}</div></div>
            </div>`
        ).join('');
    }

    // Ranking graczy (jeśli wlaczone)
    const rankEl = document.getElementById('cshop-modal-ranking');
    if (rankEl && state.opts.showPlayerRanking) {
        const playerMap = {};
        txns.forEach(t => {
            const k = t.playerName || '?';
            if (!playerMap[k]) playerMap[k] = { spent: 0, earned: 0, count: 0, amount: 0 };
            if (t.type === 'BUY')  playerMap[k].spent  += (t.price || 0);
            if (t.type === 'SELL') playerMap[k].earned += (t.price || 0);
            playerMap[k].count += 1;
            playerMap[k].amount += (t.amount || 1);
        });
        const sorted = Object.entries(playerMap).sort((a,b) => (b[1].spent + b[1].earned) - (a[1].spent + a[1].earned));
        rankEl.innerHTML = sorted.length > 0 ? `
            <div style="background:var(--bg);border:1px solid var(--border);border-radius:10px;padding:.8rem;margin-bottom:1rem;">
                <div style="font-size:.72rem;font-weight:700;color:var(--text-secondary);text-transform:uppercase;margin-bottom:.5rem;">
                    <i class="fa-solid fa-trophy" style="color:#f59e0b;"></i> Ranking graczy (ten przedmiot)
                </div>
                <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(180px,1fr));gap:.5rem;">
                    ${sorted.map(([name, d], i) => `
                        <div style="display:flex;align-items:center;gap:.5rem;padding:.4rem .6rem;background:var(--bg-card);border-radius:6px;">
                            <span style="width:22px;text-align:center;font-size:.7rem;font-weight:800;color:var(--text-secondary);">${i+1}</span>
                            <img src="https://mc-heads.net/avatar/${encodeURIComponent(name)}/22" style="width:22px;height:22px;border-radius:4px;image-rendering:pixelated;">
                            <div style="flex:1;min-width:0;">
                                <div style="font-weight:700;font-size:.78rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${escapeHtml(name)}</div>
                                <div style="font-size:.66rem;color:var(--text-secondary);">${d.count} trans. • ${d.amount} szt.</div>
                            </div>
                            <div style="text-align:right;">
                                ${d.spent > 0 ? `<div style="font-size:.72rem;font-weight:700;color:#ef4444;">-${d.spent.toFixed(0)}$</div>` : ''}
                                ${d.earned > 0 ? `<div style="font-size:.72rem;font-weight:700;color:#10b981;">+${d.earned.toFixed(0)}$</div>` : ''}
                            </div>
                        </div>
                    `).join('')}
                </div>
    ...
    // Lista transakcji
    const listEl = document.getElementById('cshop-modal-list');
    if (listEl) {
        if (txns.length === 0) {
            listEl.innerHTML = '<div style="text-align:center;padding:2rem;color:var(--text-secondary);">Brak transakcji pasujących do filtrów.</div>';
        } else {
            listEl.innerHTML = txns.map(t => {
                const ts = t.timestamp ? new Date(t.timestamp) : null;
                const tsStr = ts ? ts.toLocaleString('pl-PL', { day:'2-digit', month:'2-digit', hour:'2-digit', minute:'2-digit' }) : '?';
                const isBuy = t.type === 'BUY';
                const icon = isBuy ? 'fa-arrow-up-from-bracket' : 'fa-arrow-down-to-bracket';
                const color = isBuy ? '#ef4444' : '#10b981';
                return `
                <div style="display:flex;align-items:center;gap:.6rem;padding:.55rem .7rem;border-bottom:1px solid var(--border);font-size:.82rem;">
                    <i class="fa-solid ${icon}" style="color:${color};width:16px;text-align:center;"></i>
                    <img src="https://mc-heads.net/avatar/${encodeURIComponent(t.playerName||'Steve')}/22" style="width:22px;height:22px;border-radius:4px;image-rendering:pixelated;flex-shrink:0;">
                    <div style="min-width:100px;font-weight:700;">${escapeHtml(t.playerName||'?')}</div>
                    <div style="flex:1;color:var(--text-secondary);">${escapeHtml(t.itemName||t.category||'?')} <span style="opacity:.6;">×${t.amount||1}</span></div>
                    <div style="font-weight:800;color:${color};min-width:70px;text-align:right;">${(t.price||0).toFixed(2)}$</div>
                    <div style="font-size:.7rem;color:var(--text-secondary);min-width:80px;text-align:right;">${tsStr}</div>
                </div>`;
            }).join('');
        }
    }
...
async function loadCShopTopList(stat, containerId, color, unit) {
    ...
    try {
        ...
        if (!snap.exists()) {
            el.innerHTML = '<div style="text-align:center;padding:1.5rem;color:var(--text-secondary);font-size:.8rem;">Brak danych — plugin nie przysłał.</div>';
            return;
        }
        // Top 100 zamiast top 10 — pokazujemy top 20 w karcie + rozwijany modal z 100
        const allEntries = (snap.data().entries || []).slice(0, 100);
        if (!allEntries.length) { el.innerHTML = '<div style="text-align:center;padding:1.5rem;color:var(--text-secondary);font-size:.8rem;">Brak wpisów.</div>'; return; }

        const medals = ['🥇','🥈','🥉'];
        const visible = allEntries.slice(0, 20);
        let html = '';

        // Pasek z info i przyciskiem "Zarządzaj"
        html += `<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:.5rem;padding-bottom:.4rem;border-bottom:1px solid var(--border);">
            <span style="font-size:.68rem;color:var(--text-secondary);font-weight:700;">${allEntries.length} graczy</span>
            <button onclick="window.manageCShopTop('${stat}','${containerId}','${color}','${unit}')"
                style="padding:.25rem .6rem;background:transparent;border:1px solid ${color};border-radius:6px;color:${color};font-size:.68rem;font-weight:800;cursor:pointer;font-family:var(--font);">
                <i class="fa-solid fa-gear"></i> Zarządzaj
            </button>
        </div>`;

        html += visible.map((e, i) => {
            const val = typeof e.value === 'number' ? (Number.isInteger(e.value) ? e.value.toLocaleString('pl-PL') : e.value.toFixed(2)) : e.value;
            return `<div class="cshop-list-item" style="cursor:pointer;" onclick="window.openCShopPlayerModal('${escapeHtml(e.player||'')}')">
        ...
        if (allEntries.length > 20) {
            html += `<div style="text-align:center;padding:.5rem;">
                <button onclick="window.manageCShopTop('${stat}','${containerId}','${color}','${unit}')"
                    style="padding:.4rem 1rem;background:transparent;border:1.5px solid var(--border);border-radius:8px;color:var(--text-primary);font-size:.78rem;font-weight:700;cursor:pointer;font-family:var(--font);">
                    Zobacz wszystkie ${allEntries.length} <i class="fa-solid fa-chevron-right"></i>
                </button>
            </div>`;
        }
        el.innerHTML = html;
...
// ─── ZARZĄDZANIE TOPKĄ (dodaj/usuń/aktualizuj) ────────────────────────────────

let _cshopTopModalStat = null;

window.manageCShopTop = async function(stat, containerId, color, unit) {
    if (!requirePermission('all', 'zarządzanie topką CShop')) return;
    _cshopTopModalStat = stat;
    const snap = await getDoc(doc(db, 'cshop_top', stat));
    const entries = snap.exists() ? (snap.data().entries || []) : [];

    let modal = document.getElementById('cshop-top-modal');
    if (!modal) {
        modal = document.createElement('div');
        modal.id = 'cshop-top-modal';
        modal.className = 'modal-overlay';
        modal.setAttribute('onclick', 'if(event.target===this)this.classList.remove(\'open\')');
        modal.innerHTML = `
            <div class="modal-box" style="max-width:90vw;width:600px;max-height:92vh;">
                <div class="modal-header">
                    <div class="modal-title" id="cshop-top-modal-title">🏆 Zarządzaj topką</div>
                    <button class="modal-close" onclick="document.getElementById('cshop-top-modal').classList.remove('open')"><i class="fa-solid fa-xmark"></i></button>
                </div>
                <div class="modal-body" id="cshop-top-modal-body" style="max-height:80vh;overflow-y:auto;padding:1.2rem;"></div>
            </div>`;
        document.body.appendChild(modal);
    }
    const statLabel = stat === 'top_spent' ? 'Wydatki' : stat === 'top_earned' ? 'Zarobki' : 'Transakcje';
    const titleEl = document.getElementById('cshop-top-modal-title');
    if (titleEl) titleEl.innerHTML = '🏆 Topka: ' + statLabel;
    window._cshopTopEntries = entries;
    window._cshopTopColor = color;
    window._cshopTopUnit = unit;
    _cshopRenderTopManageBody(entries, color, unit);
    modal.classList.add('open');
};

function _cshopRenderTopManageBody(entries, color, unit) {
    const body = document.getElementById('cshop-top-modal-body');
    if (!body) return;
    ...
    let html = `
        <div style="background:var(--bg);border:1px solid var(--border);border-radius:10px;padding:.8rem;margin-bottom:1rem;">
            <div style="font-size:.72rem;font-weight:700;color:var(--text-secondary);text-transform:uppercase;margin-bottom:.5rem;">
                <i class="fa-solid fa-plus" style="color:#10b981;"></i> Dodaj lub zaktualizuj wpis
            </div>
            <div style="display:flex;gap:.5rem;flex-wrap:wrap;">
                <input type="text" id="cshop-top-add-name" placeholder="Nick gracza" autocomplete="off"
                    style="flex:1;min-width:120px;padding:.5rem .7rem;border:1.5px solid var(--border);border-radius:8px;font-size:.85rem;background:var(--bg-card);color:var(--text-primary);font-family:var(--font);">
                <input type="number" id="cshop-top-add-value" placeholder="Wartość" step="0.01"
                    style="width:100px;padding:.5rem .7rem;border:1.5px solid var(--border);border-radius:8px;font-size:.85rem;background:var(--bg-card);color:var(--text-primary);font-family:var(--font);">
                <button onclick="window._cshopTopAddEntry()"
                    style="padding:.5rem 1rem;background:#10b981;color:#fff;border:none;border-radius:8px;font-weight:800;cursor:pointer;font-family:var(--font);font-size:.85rem;">
                    <i class="fa-solid fa-plus"></i> Dodaj
                </button>
            </div>
        </div>
        <div style="font-size:.72rem;font-weight:700;color:var(--text-secondary);text-transform:uppercase;margin-bottom:.5rem;">
            ${entries.length} wpisów (klik 🗑 aby usunąć)
        </div>
        <div style="max-height:45vh;overflow-y:auto;border:1px solid var(--border);border-radius:10px;">
    `;

    if (entries.length === 0) {
        html += '<div style="text-align:center;padding:1.5rem;color:var(--text-secondary);font-size:.85rem;">Brak wpisów. Dodaj pierwszego gracza powyżej.</div>';
    } else {
        html += entries.map((e, i) => {
            const val = typeof e.value === 'number' ? (Number.isInteger(e.value) ? e.value.toLocaleString('pl-PL') : e.value.toFixed(2)) : e.value;
            return `
            <div style="display:flex;align-items:center;gap:.5rem;padding:.5rem .7rem;border-bottom:1px solid var(--border);font-size:.82rem;">
                <span style="width:24px;text-align:center;flex-shrink:0;">${i < 3 ? medals[i] : '<span style="color:var(--text-secondary);font-size:.72rem;font-weight:800;">#' + (i+1) + '</span>'}</span>
                ...
                <span style="flex:1;font-weight:700;">${escapeHtml(e.player||'?')}</span>
                <span style="font-weight:800;color:${color};">${val} ${unit}</span>
                <button onclick="window._cshopTopRemoveEntry(${i})"
                    style="padding:.3rem .5rem;background:transparent;border:1px solid rgba(239,68,68,.3);border-radius:6px;color:#ef4444;cursor:pointer;font-size:.72rem;font-family:var(--font);">
                    <i class="fa-solid fa-trash"></i>
                </button>
            </div>`;
        }).join('');
    ...
    html += `
        ...
        <button onclick="window._cshopTopSave()"
            style="margin-top:.8rem;width:100%;padding:.6rem;background:${color};color:#fff;border:none;border-radius:8px;font-weight:800;cursor:pointer;font-family:var(--font);font-size:.88rem;">
            <i class="fa-solid fa-floppy-disk"></i> Zapisz zmiany do Firestore
        </button>
    `;
    body.innerHTML = html;
}

window._cshopTopAddEntry = function() {
    const nameEl = document.getElementById('cshop-top-add-name');
    const valEl  = document.getElementById('cshop-top-add-value');
    const name = nameEl?.value?.trim();
    const val = parseFloat(valEl?.value || '0');
    if (!name) { showToast('error', 'Podaj nick gracza!'); return; }
    if (isNaN(val)) { showToast('error', 'Podaj poprawną wartość!'); return; }
    const existing = window._cshopTopEntries.findIndex(e => (e.player||'').toLowerCase() === name.toLowerCase());
    if (existing >= 0) {
        window._cshopTopEntries[existing].value = val;
        showToast('info', 'Zaktualizowano: ' + name);
    } else {
        window._cshopTopEntries.push({ rank: window._cshopTopEntries.length + 1, player: name, value: val });
        showToast('success', 'Dodano: ' + name);
    }
    _cshopRenderTopManageBody(window._cshopTopEntries, window._cshopTopColor, window._cshopTopUnit);
    if (nameEl) nameEl.value = '';
    if (valEl) valEl.value = '';
};

window._cshopTopRemoveEntry = function(idx) {
    const e = window._cshopTopEntries[idx];
    if (!e) return;
    window._cshopTopEntries.splice(idx, 1);
    showToast('info', 'Usunięto: ' + (e.player || '?'));
    _cshopRenderTopManageBody(window._cshopTopEntries, window._cshopTopColor, window._cshopTopUnit);
};

window._cshopTopSave = async function() {
    if (!_cshopTopModalStat || !window._cshopTopEntries) return;
    try {
        const sorted = [...window._cshopTopEntries].sort((a,b) => (b.value||0) - (a.value||0));
        sorted.forEach((e, i) => { e.rank = i + 1; });
        await setDoc(doc(db, 'cshop_top', _cshopTopModalStat), {
            stat: _cshopTopModalStat,
            entries: sorted,
            updatedAt: serverTimestamp()
        });
        showToast('success', 'Topka zapisana! (' + sorted.length + ' wpisów)');
        document.getElementById('cshop-top-modal')?.classList.remove('open');
        loadCShopTab();
    } catch(e) {
        showToast('error', 'Błąd zapisu: ' + e.message);
    }
...
async function loadCShopHistory() {
    ...
    if (!tbody) return;
    _cshopHistoryLoaded = false;
    tbody.innerHTML = '<tr><td colspan="7" class="table-loading"><i class="fa-solid fa-spinner fa-spin"></i> Ładowanie historii...</td></tr>';
    try {
        // Próbuj z orderBy, jeśli brak indeksu — fallback bez sortowania
        let snap;
        try {
            snap = await getDocs(query(collection(db, 'cshop_transactions'), orderBy('timestamp', 'desc'), limit(500)));
        } catch(idxErr) {
            snap = await getDocs(query(collection(db, 'cshop_transactions'), limit(500)));
        }
        _cshopAllHistory = snap.docs.map(d => ({ id: d.id, ...d.data() })).sort((a, b) => {
            const ta = cshopTimestampMs(a.timestamp);
            const tb = cshopTimestampMs(b.timestamp);
            return tb - ta;
        });
        _cshopHistoryLoaded = true;

        if (!_cshopAllHistory.length) {
            tbody.innerHTML = `<tr><td colspan="7" class="table-empty">
                <div style="text-align:center;padding:1.5rem;">
                    <i class="fa-solid fa-circle-info" style="color:#f59e0b;font-size:1.5rem;display:block;margin-bottom:.5rem;"></i>
                    <b>Brak transakcji w bazie danych</b><br>
                    <span style="font-size:.82rem;color:var(--text-secondary);">
                        Upewnij się że w pliku <code>config.yml</code> CShop jest ustawione <code>firebase.enabled: true</code>
                        oraz wpisany poprawny <code>api-key</code>.
                    </span>
                </div>
            </td></tr>`;
            return;
        ...
        // Auto-populacja kategorii z danych
        ...
        tbody.innerHTML = `<tr><td colspan="7" style="text-align:center;padding:1.5rem;color:#ef4444;font-size:.85rem;">
            <i class="fa-solid fa-triangle-exclamation"></i> Błąd ładowania: ${escapeHtml(e.message)}
        </td></tr>`;
    }
}
...
window.cshopFilterHistory = function() {
    try {
        ...
        if ((!_cshopAllHistory || _cshopAllHistory.length === 0) && !_cshopHistoryLoaded) {
        ...
        const s   = cshopNorm(document.getElementById('cshop-hist-search')?.value || '');
        const t   = document.getElementById('cshop-hist-type')?.value || '';
        const itm = cshopNorm(document.getElementById('cshop-hist-item')?.value || '');
        ...
        const filtered = _cshopAllHistory.filter(tx => {
            try {
                if (s) {
                    const p = cshopNorm(tx.playerName || tx.player || tx.nick || '');
...
window.aiExecuteAction = async function(cardTimestamp, parsed) {
    ...
    const dangerousActions = ['console_cmd'];
    if (dangerousActions.includes(parsed.action) && !requirePermission('console', 'komendy konsoli')) return;
    const advancedActions = ['gamemode', 'tp', 'fly', 'god', 'speed', 'heal', 'feed', 'clear_inv', 'whitelist', 'unwhitelist'];
    if (advancedActions.includes(parsed.action) && !requirePermission('ai_advanced', 'zaawansowane akcje AI')) return;
    if (['op','deop'].includes(parsed.action) && !requirePermission('op_manage', 'zarządzanie OP')) return;
    ...
    if (parsed.action === 'lp_permission' && !requirePermission('lp_manage', 'LuckPerms gracze')) return;
